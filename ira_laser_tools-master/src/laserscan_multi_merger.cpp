...
public:
    ...
    void scanCallback_TungNV(const sensor_msgs::LaserScan::ConstPtr& scan);
...
private:
    ...
    ros::Subscriber scan_front_subscribers;
    ros::Subscriber scan_back_subscribers;
	...
	string laser_topic_front;
	string laser_topic_back;
};
...
void LaserscanMerger::laserscan_topic_parser()
{
	...
	for (ros::master::V_TopicInfo::iterator it = topics.begin(); it != topics.end(); it++) {
		...
		// std::cout << "topic_" << it - topics.begin() << ": " << info.name << std::endl;
		ROS_INFO("laserscan_multi_merger.cpp-87-topics: %s", info.name.c_str());
	}
	...
	for(int i=0;i<tokens.size();++i)
	{
		...
		{
			// ROS_INFO("laserscan_multi_merger.cpp-87-topics: %s", topics[j].name.c_str());
			if( (tokens[i].compare(topics[j].name) == 0) && (topics[j].datatype.compare("sensor_msgs/LaserScan") == 0) )
			{
				ROS_INFO("laserscan_multi_merger.cpp-95-Add topic %s to list", topics[j].name.c_str());
	...
	{
		// ROS_INFO("laserscan_multi_merger.cpp-115");
		...
		if(input_topics.size() > 1)
		{
			...
			{
				ROS_INFO("laserscan_multi_merger.cpp-128-Subscribing to topics: %s", input_topics[i].c_str());
		...
		ROS_ERROR("laserscan_multi_merger.cpp-122-ERROR merger laserscan - %.0f laserscan received!", tmp_input_topics.size());

		// scan_front_subscribers = node_.subscribe("/sick_safetyscanners_front/scan", 1, boost::bind(&LaserscanMerger::scanCallback_TungNV,this, _1));
		// ROS_INFO("laserscan_multi_merger.cpp-133-Subscribing to topics: /sick_safetyscanners_front/scan");

		// scan_back_subscribers = node_.subscribe("/sick_safetyscanners_back/scan", 1, &LaserscanMerger::scanCallback_TungNV, this);
	}
}

LaserscanMerger::LaserscanMerger()
{
	ROS_INFO("laserscan_multi_merger.cscanCallback_pp-128-LaserscanMerger()");
	ros::NodeHandle nh("~");
    ...
    nh.param<std::string>("laser_topic_front", laser_topic_front, "/scan_front");
    nh.param<std::string>("laser_topic_back", laser_topic_back, "/scan_back");
...
{
	// ROS_INFO("laserscan_multi_merger.cpp-151-scanCallback(): %s", topic.c_str());
...
void LaserscanMerger::scanCallback_TungNV(const sensor_msgs::LaserScan::ConstPtr& scan)
{
	ROS_INFO("laserscan_multi_merger.cpp-220-scanCallback_()");
	sensor_msgs::PointCloud tmpCloud1,tmpCloud2;
	sensor_msgs::PointCloud2 tmpCloud3;
